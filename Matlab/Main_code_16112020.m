...
rbPos = 0.01;      % return on liquid asset (e.g. checking account)
rbNeg = 0.10;
rho   = 0.037;     % discount rate
ka    = 0.2;       % cost of adjustment (assuming the wage w is 1)
...
z      = [0.8,1.2]; % state values (row vector)
...
% centdiag=[centdiag;repmat(yy(1,J)+chi(1,J),I,1)]; LR*
...
%       centdiag=[centdiag;repmat(yy(n_ra,J)+chi(n_ra,J),I,1)]; LR*
end
...
% for n_ra = 2:N_ra
%     for j=2:J
%     lowdiag=[lowdiag;repmat(chi(n_ra,j),I,1)]; % this will have length I*(J-1) because the last I elements in constructing the Ith lower diagonal will be dropped anyway
%     end
% end
...
% Baux=spdiags(centdiag(1:I*J),0,I*J,I*J)+spdiags(lowdiag(1:I*J),-I,I*J,I*J)+spdiags(updiag(1:I*J),I,I*J,I*J);
% for n_ra=2:N_ra-1
%     Baux=[Baux;(spdiags(centdiag((I*J)*(n_ra -1)+1:(I*J)*n_ra),0,I*J,I*J) ...
%         +spdiags(lowdiag((I*J)*(n_ra -1)+1:(I*J)*n_ra),-I,I*J,I*J) ...
%         +spdiags(updiag((I*J)*(n_ra -1)+1:(I*J)*n_ra),I,I*J,I*J))];
% end
...
%      updiag = zeros(I*J,N_ra,Nz); % notice that endogenous nodes (b, a) are listed in the 1st dimension, while exogenous state z is listed in the 2nd
%      lowdiag = zeros(I*J,N_ra,Nz);
%      centdiag = zeros(I*J,N_ra,Nz);
...
%      for i = 1:Nz
%          for n_ra = 1:N_ra
%         centdiag(:,n_ra,i) = reshape(Y(:,:,n_ra,i),I*J,1);
%          end
%      end
% 
%      % (* reference)
%      lowdiag(1:I-1,1,:) = X(2:I,1,1,:); % exclude the 1st row i.e. b(1). This is only at a(1) (same for updiag below). Notice lowdiag(I,:)=0
%      updiag(2:I,1,:) = Z(1:I-1,1,1,:); % exclude the last row i.e. b(I). Notice upperdiag(1,:)=0
%   for n_ra = 1:N_ra
%      for j = 2:J
%         lowdiag(1:j*I,n_ra,:) = [squeeze(lowdiag(1:(j-1)*I,n_ra,:));squeeze(X(2:I,j,n_ra,:));zeros(1,Nz)]; % this stacks the columns of X in the right way (adding a 0 at the end, 
%                                                                                        % with size [1, Nz] to account for exogenous states)
%         updiag(1:j*I,n_ra,:) = [squeeze(updiag(1:(j-1)*I,n_ra,:));zeros(1,Nz);squeeze(Z(1:I-1,j,n_ra,:))];
%      end
%   end
% 
% % now we need to re-arrange diags for using spdiags
% center_diag = squeeze(centdiag(:,1,:));
% upper_diag = squeeze(updiag(:,1,:));
% lower_diag = squeeze(lowdiag(:,1,:));
% for n_ra = 2:N_ra
%     center_diag = [center_diag; squeeze(centdiag(:,n_ra,:))];
%     upper_diag  = [upper_diag; squeeze(updiag(:,n_ra,:))];
%     lower_diag  = [lower_diag; squeeze(lowdiag(:,n_ra,:))];
% end
%  
%   AA1=spdiags(center_diag(:,1),0,I*J*N_ra,I*J*N_ra)+spdiags(upper_diag(:,1),1,I*J*N_ra,I*J*N_ra)+spdiags(lower_diag(:,1),-1,I*J*N_ra,I*J*N_ra); % notice that when filling the upper diagonal
...
%   AA2=spdiags(center_diag(:,2),0,I*J*N_ra,I*J*N_ra)+spdiags(upper_diag(:,2),1,I*J*N_ra,I*J*N_ra)+spdiags(lower_diag(:,2),-1,I*J*N_ra,I*J*N_ra);
%   AA = [AA1, sparse(I*J*N_ra,I*J*N_ra); sparse(I*J*N_ra,I*J*N_ra), AA2]; % this completes the I*J matrixes to bring them to I*J*Nz
...
vstarAux = zeros(10,2,Nx); % 2 stays for Nz while 10 for N_ra. The Aux elements are those defined on the grid for x
bAdjAux  = zeros(10,2,Nx);
aAdjAux  = zeros(10,2,Nx);
vAdj = zeros(10,2,NaP); % for each x in the grid, search over a grid for a' - backing out b' = x - a' - ka (see ** reference) - to find the maximum. This vadj
...
% transforming bAdj, aAdj into a vector
bAdj1_ = bAdj1{1};
bAdj2_ = bAdj2{1};
aAdj1_ = aAdj1{1};
aAdj2_ = aAdj2{1};
...
fprintf('Made it !!')
cd '16-11-2020\kappa2'
save('DATA_16112020') % to avoid going through the iteration again.
...
set(gcf,'PaperPosition',[0 0 15 5])
subplot(2,2,1)
surf(b,a,bAdj_plot(:,:,5,1)')
xlabel('Liquid Wealth (b)','FontSize',14, 'interpreter','latex')
title('Liquid Adj. Target, Low Income')

subplot(2,2,2)
surf(b,a,bAdj_plot(:,:,5,2)')
xlabel('Liquid Wealth (b)','FontSize',14, 'interpreter','latex')
title('Liquid Adj. Target, High Income')

subplot(2,2,3)
surf(b,a,bAdj_plot(:,:,1,1)')
xlabel('Liquid Wealth (b)','FontSize',14, 'interpreter','latex')
title('Liquid Adj. Target, Low Income')

subplot(2,2,4)
surf(b,a,bAdj_plot(:,:,1,2)')
xlabel('Liquid Wealth (b)','FontSize',14, 'interpreter','latex')
title('Liquid Adj. Target, High Income')
...
set(gcf,'PaperPosition',[0 0 15 5])
subplot(2,2,1)
surf(b,a,aAdj_plot(:,:,5,1)')
xlabel('Liquid Wealth (b)','FontSize',14, 'interpreter','latex')
title('Illiquid Adj. Target, Low Income')

subplot(2,2,2)
surf(b,a,aAdj_plot(:,:,5,2)')
xlabel('Liquid Wealth (b)','FontSize',14, 'interpreter','latex')
title('Illiquid Adj. Target, High Income')

subplot(2,2,3)
surf(b,a,aAdj_plot(:,:,1,1)')
xlabel('Liquid Wealth (b)','FontSize',14, 'interpreter','latex')
title('Illiquid Adj. Target, Low Income')

subplot(2,2,4)
surf(b,a,aAdj_plot(:,:,1,2)')
xlabel('Liquid Wealth (b)','FontSize',14, 'interpreter','latex')
title('Illiquid Adj. Target, High Income')
print -dpng adjTarget_a.png
...
% ojo: grillas son diferentes -> importante para el share.

a_final = aAdj + aaaa.*(aAdj==0);
b_final = bAdj + bbbb.*(bAdj==0);
wealth = a_final + b_final;
share_a = a_final./wealth;

share_a = reshape(share_a,I*J*N_ra*Nz,1);
wealth = reshape(wealth,I*J*N_ra*Nz,1);
...
for z = 1:Nz
    for n_ra = 1:N_ra-1
        share_a(I*J*n_ra*z +1:I*J*(n_ra+1)*z) = sort(share_a(I*J*n_ra*z +1:I*J*(n_ra+1)*z));
        wealth(I*J*n_ra*z +1:I*J*(n_ra+1)*z) = sort(wealth(I*J*n_ra*z +1:I*J*(n_ra+1)*z));
    end
end

plot(wealth((2*I*J)+5:3*I*J),share_a((2*I*J)+5:3*I*J))